...
const STATUS_LABELS = { NEW: "Nou", INFO_SENT: "Informació enviada", FOLLOW_UP: "En seguiment", REPLIED: "Ha contestat", PENDING_DECISION: "Pendent de decisió", BOOKING_CONFIRMED: "Reserva confirmada", CONTACT_LATER: "Contactar més endavant", CANCELLED: "Reserva cancel·lada", LOST: "Perdut" };
...
document.addEventListener("click", async (event) => {
  ...
  const bookedReservationButton = event.target.closest("[data-open-booked-reservation]");
  if (bookedReservationButton) {
    window.dispatchEvent(new CustomEvent("travelflow:open-client-reservation", { detail: { clientId: bookedReservationButton.dataset.clientId, tripId: bookedReservationButton.dataset.openBookedReservation } }));
    return;
  }
  const tripBookingButton = event.target.closest("[data-confirm-trip-booking]");
...
document.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (formType === "booking") {
      ...
      const booking = await confirmBooking(lead, { tripId: data.tripId, dui: formData.has("dui"), priceConcepts });
      window.dispatchEvent(new CustomEvent("travelflow:open-client-reservation", { detail: booking }));
      window.dispatchEvent(new CustomEvent("travelflow:tasks-updated"));
      return;
    }
