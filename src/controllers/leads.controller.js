...
function renderDetail(lead, activities, tasks) {
  ...
  <header class="lead-detail-hero"><div class="lead-detail-hero__avatar">${initials(lead.fullName)}</div><div class="lead-detail-hero__content"><span class="section-kicker">Futura viatgera</span><h1>${escapeHtml(lead.fullName)}</h1><div class="lead-detail-hero__meta"><span class="lead-channel lead-channel--${String(lead.channel).toLowerCase()}">${CHANNEL_LABELS[lead.channel]}</span><span>${escapeHtml(trip)}</span></div>${renderContactLinks(lead)}</div></header>
