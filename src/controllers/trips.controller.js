...
function renderTripRows(trips) {
  if (!trips.length) return `<div class="leads-empty"><h2>Encara no hi ha viatges</h2><p>Crea la primera etiqueta de viatge.</p></div>`;
...
function renderTripsView(trips) {
  return `<section class="trips-management-page"><header class="page-heading"><div><span class="section-kicker">Configuració comercial</span><h1>Etiquetes de viatge</h1><p>Crea i gestiona els viatges que es poden assignar als leads.</p></div><button class="primary-button primary-button--compact" type="button" data-create-trip>+ Nou viatge</button></header><section class="trips-management-summary"><div><strong>${trips.length}</strong><span>Etiquetes totals</span></div><div><strong>${trips.filter((trip) => !trip.startDate || !trip.endDate).length}</strong><span>Amb dates pendents</span></div></section><section class="trips-management-card"><div class="trips-management-head"><span>Viatge</span><span>Dates i tancament</span><span></span></div><div>${renderTripRows(trips)}</div></section></section>`;
}
...
async function showTripsView() { const root = getRoot(); if (!root) return; root.innerHTML = '<section class="trips-management-page"><div class="leads-loading"><span class="leads-loading__spinner"></span><p>Preparant viatges...</p></div></section>'; try { await seedInitialTrips(); root.innerHTML = renderTripsView(await getTrips({ force: true })); } catch (error) { root.innerHTML = `<section class="trips-management-page"><div class="leads-error">${getTripErrorMessage(error)}</div></section>`; } }
