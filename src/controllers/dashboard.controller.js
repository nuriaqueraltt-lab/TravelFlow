...
function renderTripCards(trips) {
  ...
  return `<section class="dashboard-trips"><header class="dashboard-trips__header"><div><span class="section-kicker">Control comercial</span><h2>Pròxims viatges</h2></div><p>Clica una targeta per veure només els leads d’aquell viatge.</p></header><div class="dashboard-trip-grid">${upcoming.map((trip) => {
    ...
    return `<button class="dashboard-trip-card" type="button" data-dashboard-trip="${trip.id}" style="--trip-image:url('${escapeHtml(imageUrl)}')"><span class="dashboard-trip-card__overlay"></span><span class="dashboard-trip-card__top"><span class="dashboard-trip-card__date">${formatDate(`${trip.startDate}T12:00:00`)}</span><span class="dashboard-trip-card__closing ${trip.closingDate ? "" : "is-pending"}">${closingText}</span></span><span class="dashboard-trip-card__content"><strong>${escapeHtml(trip.name.replace(/^\d{4}\s*-\s*/, ""))}</strong><span>Obrir el control comercial del viatge</span></span><span class="dashboard-trip-card__arrow">Veure leads →</span></button>`;
  }).join("")}</div></section>`;
}

function renderDashboard(tasks, trips) {
...
export async function showDailyDashboard({ force = false } = {}) {
  ...
  if (!force && cacheValid) {
    root().innerHTML = renderDashboard(dashboardData.tasks, dashboardData.trips);
...
window.addEventListener("travelflow:tasks-updated", invalidateDashboardCache);
