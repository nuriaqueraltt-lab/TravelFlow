...
async function createLeadSupportRecords({
  leadId,
  fullName,
  tripLabels,
  channel,
  ...
  batch.set(doc(collection(db, "activities")), {
    ...
    description: `Nova consulta rebuda per ${entryLabel || channel}.`,
    channel,
    source,
    createdBy: userId,
...
export async function createLead(input) {
  ...
  if (input.entryDate && input.lastContactDate && input.lastContactDate < input.entryDate) {
    throw new Error("CONTACT_DATE_ORDER");
  }
  ...
  await setDoc(leadRef, {
    ...
    nextActionTitle: "Primer seguiment pendent",
  ...
  createLeadSupportRecords({
    ...
    console.error("El lead s'ha creat, però no s'han pogut completar els registres auxiliars:", error);
  });
